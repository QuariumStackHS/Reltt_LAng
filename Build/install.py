    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(os.path.join(
                app.config['UPLOAD_FOLDER'], constructor+".o"))
            if Mods.query.filter(Mods.name == constructor).first() is None:
                h = Mods(constructor, Dev, Version, OS, os.path.join(
                    app.config['UPLOAD_FOLDER'], constructor+".o"), constructor, Desc)
                db.session.add(h)
                db.session.commit()
...
def build_Reltt():
    ...
    os.system(
        "g++ -w -std=c++17 -c ../Reltt/Reltt_Linked_List.cpp -o ../Reltt/Cache/Reltt_Linked_List.o")
    os.system(
        "g++ -w -std=c++17 -c ../Reltt/Reltt_Value.cpp -o ../Reltt/Cache/Reltt_Value.o")
